...
- Composing objects as VALUES (`frameforge_sdk.objects`): a builder method returns the
  builder, so `page.rect(...)` gives you back nothing you can keep working on. `Obj` is
  that object as an immutable value, CLOSED under its own operations — each returns
  another `Obj`, so a shape can be built once, refined, and re-placed:
      from frameforge_sdk import Obj
      card = Obj.rect().styled(fill="#1C1C1A", stroke="#C2410C").with_(radius=8)
      page.extend([card.at(cell).named(f"card-{i}")
                   for i, cell in enumerate(content.column(3, gap=12))])
  Constructors: `Obj.of(type, **fields)` plus named `Obj.rect/text/ellipse/circle/line/
  polyline/path/image/group`. Operations: `.with_(**fields)` `.without(*keys)` `.at(box)`
  `.move(dx, dy)` `.styled(fill=...)` `.named(id)` `.grouped_with(*others)` and `a + b`
  (group; an ANONYMOUS group absorbs so `a + b + c` is flat, a NAMED one nests because
  its id is addressable). Queries `.type` `.id` `.box` (a `Box`) `.children` (`Obj`s).
  It IS a dict: `page.add(obj)`, validation, recolor and both serializers take it
  unchanged, and mutating it raises. `as_obj(one)` / `objects(many)` lift widget, chart
  and `svg_to_objects` output into the same vocabulary.
- Named contract keys, generated (`frameforge_sdk._fieldtypes`): a `total=False`
  `TypedDict` per contract class — 125 classes, 1974 keys — which is the `Unpack`
  (PEP 692) vocabulary behind every `**fields` bag, so a typo is a type error where it
  is typed. `NAMED_KEYS` is the roster as data (use it to advertise what you can set)
  and `unnamed_keys()` reads the remaining gap from the live contract; it is empty.
  Generated by `tooling/gen_field_types.py` with a drift gate, because a hand-written
  mirror of a closed contract rots. `fieldsets.py` (above) remains the RUNTIME-checked
  form; these are the typing-time form of the same idea.
- Design diagnostics in the author's coordinates (`explain_validation_report`): the
  companion to `explain_validation_error` above, for the other reporter.
  `validate_static_rules` and `lint_deprecations` both answer with a `ValidationReport`
  whose every `Issue` carries a JSON pointer, which is what the provenance map is keyed
  by — so `explain_validation_report(report, doc.provenance_map())` returns an
  `ExplainedReport` of `DesignProblem`s (`rule_id`, `severity`, `path`, `message`,
  `site`), with `.errors`/`.warnings`/`.notices`, `.format()` and a JSON-safe
  `.to_dict()`. `write(fail_on_error=True)` attaches it to its `StaticValidationError`
  automatically. File and line still need `capture_provenance=True`.
